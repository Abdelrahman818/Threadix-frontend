'use client';

import { useRouter } from 'next/navigation';
import { useState, useEffect } from 'react';
import { END_POINT, API_BASE_URL } from '@/config';
import { fetchDemoFeaturedProducts, getDemoImageSrc, isDemoMode } from '@/lib/demoMode';

const Featured = () => {
  const router = useRouter();
  const [products, setProducts] = useState([]);

  useEffect(() => {
    if (isDemoMode) {
      fetchDemoFeaturedProducts()
        .then(setProducts)
        .catch(error => console.error(error.message));
      return;
    }

    fetch(END_POINT.GET_FEATURED, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
    })
      .then(res => res.json())
      .then(json => setProducts(json.data))
      .catch(error => console.error(error.message));
  }, []);

  return (
    <section className="featured">
      <h2>Featured Drops</h2>

      <div className="product-grid">
        {products?.map((p) => (
          <div key={p._id} onClick={() => router.push(`/shop/item/${p._id}`)} className="product-card">
            <div className="product-img">
              <img src={isDemoMode ? getDemoImageSrc(p.images) : `${API_BASE_URL}${p.images[0]}`} alt={p.title || "Product"} />
            </div>
            <h3>{p.title && p.title.charAt(0).toUpperCase() + p.title.slice(1)}</h3>
              <p className="price">{p.salePrice} LE</p>
          </div>
        ))}
      </div>
    </section>
  )
}

export default Featured;
